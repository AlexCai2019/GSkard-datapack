...
$bossbar add custom_buff:pojia_$(UUID) [{text:"破甲",color:"aqua",bold:true}]
$bossbar set custom_buff:pojia_$(UUID) name [{text:"破甲",color:"aqua",bold:true},{text:" | ",color:"#6796E6",bold:false},{text:"时间剩余:",color:gray,bold:false},{score:{name:"temp",objective:temp_2},color:green,bold:false},{text:".",color:green,bold:false},{score:{name:"temp",objective:temp_3},color:green,bold:false},{text:"秒",color:gray,bold:false},{text:" | ",color:"#6796E6",bold:false},{text:"-40%护甲值",color:red,bold:false}]
$bossbar set custom_buff:pojia_$(UUID) color blue
$bossbar set custom_buff:pojia_$(UUID) players @s
$bossbar set custom_buff:pojia_$(UUID) visible true

$execute as @s[tag=!bossbar_max_PoJia] store result bossbar custom_buff:pojia_$(UUID) max run scoreboard players get @s PoJia
...
$execute store result bossbar custom_buff:pojia_$(UUID) value run scoreboard players get @s PoJia
# 判断当前值是否比上一刻大
