...
$bossbar add custom_buff:beipan_$(UUID) [{text:"背叛",color:"dark_gray",bold:true}]
$bossbar set custom_buff:beipan_$(UUID) name [{text:"背叛",color:"dark_gray",bold:true},{text:" | ",color:"#6796E6",bold:false},{text:"时间剩余:",color:gray,bold:false},{score:{name:"temp",objective:temp_2},color:green,bold:false},{text:".",color:green,bold:false},{score:{name:"temp",objective:temp_3},color:green,bold:false},{text:"秒",color:gray,bold:false},{text:" | ",color:"#6796E6",bold:false},{text:"你攻击生物后对队友造成1♥魔法伤害",color:red,bold:false}]
$bossbar set custom_buff:beipan_$(UUID) color purple
$bossbar set custom_buff:beipan_$(UUID) players @s
$bossbar set custom_buff:beipan_$(UUID) visible true

$execute as @s[tag=!bossbar_max_BeiPan] store result bossbar custom_buff:beipan_$(UUID) max run scoreboard players get @s BeiPan
...
$execute store result bossbar custom_buff:beipan_$(UUID) value run scoreboard players get @s BeiPan
# 判断当前值是否比上一刻大
